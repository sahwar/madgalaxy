...
var mongoosePost  = require('./models/post.js');
 ...
 * @return {null}            null
 */
function savePost(postToSave) {    //TODO - NEED TO FIX THIS

  //ES is sometimes updating some articles and not updating others - it is realted to how the docs are saved
  mongoose.model('postModel').findOne({title: postToSave.title}).exec(function (err, results){
    if(results){
      var preserveID = results._id;
      postToSave._id = preserveID;
      mongoose.model('postModel').remove( {title: postToSave.title} ).exec(function (err, result){
...
function parsePost(post) {
  ...
  if (post && post.title != '' && post.title != 'No title' && post.title && post.description) {
    ...
    console.log('Not enough info to add post to database - blank title, blank content, etc.');
  }
}
...
function setup(){
  for (var site in config){
    build(config[site], function() {});
  }
}
mongoosePost.setupES(function(){
  setup();
});
var minutes = 30;
