...
var debug = {
    ...
    post_saver: require('debug')('feedreader:post_saver'),
        ...
        if (results) {
            ...
            mongoose.model('postModel').remove({
                ...
                postToSave.save(function (err, products, numberAffected) {
                    if (err) {
                        debug.post_saver('Error saving document: ' + err);
                    } else {
                        debug.post_saver(('rewriting document: ' + postToSave.title));
                    }
                });
            });
         ...
         */
            ...
            postToSave.save(function (err, products, numberAffected) {
                if (err) {
                    ...
                    debug.post_saver(('Adding the post to DB: ' + postToSave.title));
                }
            });
        }
    });
...
function parsePost(post) {
    ...
    if (post && post.title != '' && post.title != 'No title' && post.title && post.description) {
        ...
        debug.post_saver('Doc does not have all required data: ' + JSON.stringify(post.link));
    }
}
...
setInterval(function () {
    debug.feedreader("//---------------------This is the periodic update--------------------//");
    setupES(function () {
        setup();
    });
}, theInterval);