...
/*
  ...
  1 - After the articles are brought in by build(), they are sent to parsePost()
  2 - parsePost() then makes sure the document has everything that is required such as a title, description, etc.
  ...
  3 - parsePost() then sends the object on to savePost()
  4 - savePost() checks to see if the article is already in the DB (by searching for the same title)
          If it is,
...
function savePost(postObject) {
    ...
    findPost(postObject, function(err, results) {
        //It does exist - so I will delete it and then save it again with the old ID and number of views
        if (results) {
...
function parsePost(post) {
    ...
    if (post && post.title != '' && post.title != 'No title' && post.title && post.description) {
        contentParser.parseText(post.title, function(err, parsedTitle) {
            contentParser.parseText(post.description, function(err, parsedDescription) {
 ...
 *
 * use the default es driver as mongoosastic does not support this
 */
