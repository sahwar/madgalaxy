...
var natural = require('natural');  //EXPERIMENTAL FEATURE!
var RSS = require('rss');
...
function parsePost(post, defaultTags) {
  if (post && post.title != '' && post.title != 'No title') {
    ...
    TfIdf = natural.TfIdf,
    tfidf = new TfIdf();
    tfidf.addDocument(post.description);
    tfidf.addDocument('This is required due to a bug. This does not affect results');
    var tagsForArticle = (tfidf.listTerms(0));
    /*
    tagsForArticle is in the format:
      term: blah, tfidf: someNum
     */

    /*
    This next block determines the average value for the tags given, and then adds the tag to tags[] if it is greater than the average
     */
    var averageVal = 0;
    for (var itemNum in tagsForArticle){
      averageVal = parseFloat(averageVal + parseFloat(tagsForArticle[itemNum].tfidf) );
    }
    averageVal = averageVal/(tagsForArticle.length);
    for (var item in tagsForArticle){
      if(tagsForArticle[item].tfidf > averageVal){
        tags.push(tagsForArticle[item].term);
      }
    }
 ...
 */
function build(defaultTags, blog, cb) {
  var counter = 1;
