...
function tagArticle(articleTitle, articleDescription, callback) {
    var tags = [];
    articleTitle = slugify(articleTitle).split(" ");
    articleDescription = slugify(articleDescription).split(" ");
    ...
    function createTagsArray() {
        ...
        for (var tag in config.tags) {
            ...
            for (var tagVarient in config.tags[tag]) {
                //if 2nd level tag is in title and not alreday in tags[]
                if (articleTitle.indexOf(config.tags[tag][tagVarient]) != -1 && tags.indexOf(tag) == -1) {
                    tags.push(tag);
                }
                //if 2nd level tag is in description and not alreday in tags[]
