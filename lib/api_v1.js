...
function _helper_userTagAcceptable(tag){
  var acceptableTags = [];
  for(tag in config.tags){
    acceptableTags.push(tag);
    for(tag2 in config.tags[tag]){
      acceptableTags.push(tag2);
    }
  }
  return _helper_inArray(tag, acceptableTags);
 ...
 * @param {Function} callback err, results
 */
function addUserTagToArticle(page, id, tag, callback) {
  mongoose
    .model('postModel')
    .findById(id, function(err, doc) {
      if(! _helper_inArray(tag, doc.userAddedTags)){
